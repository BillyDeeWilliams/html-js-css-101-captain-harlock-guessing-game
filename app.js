...
//populate answer key with correct answers
ynanswers.push('no');
ynanswers.push('yes');
ynanswers.push('no');
ynanswers.push('yes');
...
var qsremain = ynquestions.length; //number of questions reamining equal to total number of questions
...
function yNquestionsgen () {
  for ( var i = 0 ; i < ynquestions.length ; i++ ){ //do this for as many times as there are questions
    userguesses.push(prompt(ynquestions[i]).toLowerCase()); // toLowerCase to controll comparison populate array for user answers input
...
yNquestionsgen (); //call function for generating y n questions then asking for answers and checking those ansers
...
questionSeven (); //function call for last question with many possible correct solutions
